...
BOOL Show_Short_String (BYTE **data, const char *str)
{
    int len;
...
void Show_String (BYTE **data, const char *str)
{
    if (Show_Short_String (data, str))
        M_print ("\n");
}

void Show_String_A (BYTE **data, const char *fmt, ...)
{
    char buf[2048];
    va_list args;
    ...
    switch ( subcmd ) {
        ...
        case META_SRV_MORE:
            ...
            if (Chars_2_Word (data) != (WORD) -1)
                M_print (SERVCOL "%s%d     \t", i18n (527, "Age             : "), Chars_2_Word (data));
            else
                M_print (SERVCOL "%s%s\t", i18n (527, "Age             : "), i18n (526, "Not Entered"));
            data+=2;
            if (*data == 1)
            ...
            {
                M_print ("%s%02d.%s. %4d \t", i18n (532, "Born            : "), *(data + 2), i18n (-1, Month[*(data + 1)]), *(data) + 1900);
                newline = "\n";
            }
            data += 3;
        ...
        case META_SRV_WORK:
            ...
            if ( 0 != Chars_2_Word( data) ) {
                if ( Get_Country_Name( Chars_2_Word(data) ) != NULL )
                    M_print (SERVCOL "%s%s\n", i18n (514, "Work Country : "), Get_Country_Name (Chars_2_Word(data)));
                else
                    M_print (SERVCOL "%s%d\n", i18n (513, "Work Country Code : "), Chars_2_Word (data));
                newline = "";
            }
            M_print( "%s", newline);
            data += 2;
            ...
            if ( 0 != Chars_2_Word( data ) ) 
                M_print (SERVCOL "%s%s\n", i18n (516, "Occupation    : "), Get_Occupation (Chars_2_Word (data)));
            data += 2;
        ...
        case META_SRV_GEN:
            ...
            if ( Get_Country_Name( Chars_2_Word(data) ) != NULL )
                M_print ("%s%s\t", i18n (511, "Country        : "), Get_Country_Name (Chars_2_Word (data)));
            else
