...
/* AUTOGENERATED by enssl dhparam -5 -C */
static DH *get_dh512()
{
...
int ssl_sockread (Connection *conn, UBYTE *data, UWORD len_p)
{
    ...
    if (conn->ssl_status == SSL_STATUS_HANDSHAKE)
    {
        ...
        Debug (DEB_SSL, "ssl_sockread calling handshake [ret=%d errno=%d]", rc, errno);
        errno = EAGAIN;
        return -1;
    }
    if (conn->ssl_status != SSL_STATUS_OK)
...
#else
    Debug (DEB_SSL, "SSL_read pre");
    len = SSL_read (conn->ssl, data, len_p);
    Debug (DEB_SSL, "SSL_read post");
    if (len > 0)
    {
        Debug (DEB_SSL, "read %d bytes from %s", len, contact->nick);
...
int ssl_sockwrite (Connection *conn, UBYTE *data, UWORD len_p)
{
    ...
    if (len > 0)
    {
        Debug (DEB_SSL, "sent %d/%d bytes to %s", len, len_p, contact->nick);
        return len;
    }
    tmp = SSL_get_error (conn->ssl, len);
    if (tmp != SSL_ERROR_NONE)
