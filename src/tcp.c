/*********************************************
**********************************************
This file handles TCP client-to-client
communications.

Author : James Schofield (jschofield@ottawa.com)
...
int Open_TCP_Connection ( CONTACT_PTR cont )
{
  ...
  if ( uiG.Verbose )
    M_print ("Versuche es mit %s...", inet_ntoa (sin.sin_addr));
                          ...
                          sizeof (struct sockaddr)) < 0)
  {
    ...
    if ( uiG.Verbose )
      M_print (" fehlgeschlagen.\nVersuche es mit %s...", inet_ntoa (sin.sin_addr));
...
int Wait_TCP_Init_Ack ( SOK_T sok )
{
  ...
  {
    M_print ("Fehler beim Epfang eines TCP-Paketes.\n");
    return (-1);
  }
...
void Handle_TCP_Comm ( UDWORD uin )
{
  ...
  while ( M_Is_Set( cont->sok ) && i <= TCP_MSG_QUEUE )
  {
    ...
    else 
    {
      ...
      {
        R_undraw ();
        M_print("\nEingehendes Paket von " CONTACTCOL "%lu" NOCOL "...\n", uin);
        Hex_Dump ( pak, size );
        M_print ("\n");
        R_redraw ();
      }
      ...
      { 
        ...
        switch ( Chars_2_Word ( pak->cmd ) )
        {
          case TCP_CMD_ACK:
            ...
            {
              R_undraw ();
              M_print ("Empfangsbestätigung für Nachricht (seq %04X) von %s",
                                                      seq_in, cont->nick );
              R_redraw ();
            }
            break;
          ...
          case TCP_CMD_CANCEL:
            ...
            {
              R_undraw ();
              M_print ("Eingehende Nachtricht (seq %04X) von %s abgebrochen",
                                                      seq_in, cont->nick );
              R_redraw ();
            }
            break;
  ...
  while ( ( msg_in = msg_queue_pop ( tcp_rq ) ) != NULL )
  {
    ...
    switch ( Chars_2_Word ( pak->sub_cmd ) )
    {
      ...
      case FILE_MESS:
        Send_TCP_Ack ( cont->sok, Chars_2_Word(pak->seq),
                                         Chars_2_Word(pak->sub_cmd ), FALSE );
	break;
      ...
      default:
        ...
        Send_TCP_Ack ( cont->sok, Chars_2_Word(pak->seq),
                                          Chars_2_Word(pak->sub_cmd ), TRUE );
	if ( uiG.Verbose )
	  M_print ("Sende TCP-Bestätigung an " CONTACTCOL "%s" NOCOL ".\n", cont->nick);
    }
    free ( pak );
    free ( msg_in );
    R_redraw ();
    ...
    if ((queued_msg = msg_queue_pop( tcp_sq )) != NULL)
    {
        ...
        if (queued_msg->attempts <= MAX_RETRY_ATTEMPTS)
        {
            ...
            else
            {
              ...
              if ( (Chars_2_Word(pak->msg_type) && 
                                   TCP_AUTO_RESPONSE_MASK) == 0 )
              {
