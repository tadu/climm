...
static jump_f
    ...
    CmdUserTogIgnore, CmdUserTogInvis, CmdUserTogVisible, CmdUserAdd,
    CmdUserRemove, CmdUserAuth, CmdUserURL, CmdUserSave, CmdUserTabs,
    CmdUserLast, CmdUserHistory, CmdUserFind, CmdUserUptime,
...
static jump_t jump[] = {
    ...
    { &CmdUserHistory,       "h",            NULL, 2,   0 }, 
    { &CmdUserHistory,       "history",      NULL, 2,   0 }, 
    { &CmdUserHistory,       "historyd",     NULL, 2,   1 },
    { &CmdUserFind,          "find",         NULL, 2,   0 }, 
    { &CmdUserFind,          "finds",        NULL, 2,   1 }, 
    { &CmdUserUptime,        "uptime",       NULL, 2,   0 },
    { &CmdUserPeer,          "peer",         NULL, 0,   0 },
    { &CmdUserPeer,          "tcp",          NULL, 0,   0 },
    { &CmdUserPeer,          "file",         NULL, 0,   4 },
    { &CmdUserPeer,          "accept",       NULL, 0,  22 },
...
static JUMP_F(CmdUserHelp)
{
    ...
    else if (what == 2)
    {
        ...
        M_printf (COLMESSAGE "%s <contact> [<last> [<count>]]" COLNONE "\n\t" COLINDENT "%s" COLEXDENT "\n",
                  CmdUserLookupName ("history"),
                  i18n (2383, "View your last messages of <contact>. Type 'h' as short command."));
        M_printf (COLMESSAGE "%s <contact> <date> [<count>]" COLNONE "\n\t" COLINDENT "%s" COLEXDENT "\n",
                  CmdUserLookupName ("historyd"),
                  i18n (2395, "View your messages of <contact> since <date>."));
        M_printf (COLMESSAGE "%s <contact> <pattern>" COLNONE "\n\t" COLINDENT "%s" COLEXDENT "\n",
                  CmdUserLookupName ("find"),
    ...
    else if (what == 5)
    {
        ...
        M_printf (COLMESSAGE "%s deny [<uin|nick>] [<id>] [<reason>]" COLNONE "\n\t" COLINDENT "%s" COLEXDENT "\n",
                  CmdUserLookupName ("peer"),
                  i18n (2369, "Refuse an incoming file transfer from <uin> or <nick>."));
...
static JUMP_F(CmdUserPeer)
{
    ...
    while (data && (s_parsenick_s (&args, &cont, MULTI_SEP, NULL, conn) || (data & 16)))
    {
        data &= 15;
        switch (data)
        {
            ...
            case 7:
                {
                    ...
                    s_parseint (&args, &seq);
                    if (!(event = QueueDequeue2 (conn, QUEUE_ACKNOWLEDGE, seq, cont ? cont->uin : 0)) || !(revent = event->rel) ||
 ...
 *      <count>   Number of shown message. <contact> -1 20 list the first
...
static JUMP_F(CmdUserHistory)
{
    ...
    char timestr[MAX_STR_BUF], timeformat[MAX_STR_BUF];
    BOOL isMsg = FALSE, isEmptyMsg = FALSE;

    FILE *logfile;
    long *fposArray = NULL, fposLen = 0, len;
    char *line, buffer[LOG_MAX_PATH+1], *target = buffer;
    int fposCur = 0, fd;
    ...
    if (!s_parsenick (&args, &cont, &contr, conn))
    {
        if (s_parse (&args, &line))
            M_printf (i18n (1061, "'%s' not recognized as a nick name.\n"), line);
        else
            M_printf (i18n (2387, "%s <contact> [<last> [<count>]] -- Show history of <contact>.\n"),
                      CmdUserLookupName ("history"));
        return 0;
    }
    ...
    while (s_parse_s (&args, &p, MULTI_SEP))
    {
        switch (n)
        {
            case 0:
                ...
                if (data == 1)
                {
                    ...
                    {
                        M_printf (i18n (2396, "Parameter '%s' has a wrong date format, it has to be ISO 8601 compliant. Try '2004-31-01' or '2004-31-01T23:12:05'.\n"), p);
                        return 0;
                    }
                }
                else
                {
                    if (!sscanf (p, "%d", &msgMin))
                        msgMin = DEFAULT_HISTORY_COUNT;
                }
                break;
            case 1:
                if (!sscanf (p, "%d", &msgNum))
    ...
    {
        M_printf (i18n (2385, "Couldn't open %s for reading: %s (%d).\n"),
        ...
        while ((line = UtilIOReadline (logfile)))
        {
            if (*line == '#')
            {
                /* get len to correct the file position to the
                   beginning of line */
                len = (long)strlen (line) + 1;
                line++;
                n = 0;
                while (s_parse (&line, &p))
                {
                    switch (n)
                    {
                        case 2: 
    ...
    sprintf (timeformat, COLDEBUG "%s" COLNONE "%s",
             i18n (2393, "%m/%d/%Y"), i18n (2394, " %I:%M %p"));
                ...
                switch (n)
                {
                    ...
                    case 2:
                            ...
                            (p[0] == '-' && p[1] == '>'))
                        {
                            ...
                            M_printf ("%s " COLNONE "[%4d] ", timestr, msgCount);

                            if (*p == '<')
                                M_printf (COLINCOMING "<- ");
                            else
                                M_printf (COLSENT "-> ");
                        }
                        break;
...
static JUMP_F(CmdUserFind)
{
    ...
    char *line = NULL, *pattern, *p = NULL, c;
    ...
    if (!s_parsenick (&args, &cont, &contr, conn))
    {
        ...
        {
            M_printf (i18n (2388, "%s <contact> <pattern> -- Search <pattern> case insensitive in history of <contact>.\n"),
                      CmdUserLookupName ("find"));
            M_printf (i18n (2389, "%s <contact> <pattern> -- Search <pattern> case sensitive in history of <contact>.\n"),
                      CmdUserLookupName ("finds"));
        }
        return 0;
    }
    ...
    pattern = args;
    while (*pattern && strchr (DEFAULT_SEP, *pattern))
        pattern++;
    if (*pattern == '"')
        pattern++;
    patternLen = strlen (pattern);
    while (patternLen > 0 && strchr (DEFAULT_SEP, pattern[patternLen - 1]))
        patternLen--;
    ...
    if (!patternLen)
    {
        M_printf (i18n (2390, "No or empty pattern given.\n"));
        return 0;
    }
    ...
    while (!doRead || (line = UtilIOReadline (logfile)))
    {
        ...
        while ((line = UtilIOReadline (logfile)))
        {
            ...
            if (!msg) 
            {   
                msg = (char*) malloc (size = MEMALIGN (strlen (line) + 2));
                *msg = '\0';
                if (!data)
                    msgLower = (char*) malloc (size);
            }
            len = strlen (msg);
            if (size < len + strlen (line) + 2)
            {
                msg = realloc (msg, size = MEMALIGN (len + strlen (line) + 2));
                if (!data)
    ...
    M_printf (i18n (2386, "Found %s%d%s matches.\n"), COLMESSAGE, matchCount, COLNONE);
...
static JUMP_F(CmdUserConn)
{
    ...
    if (!data)
    {
        if (!s_parse (&args, &arg1))       data = 1;
        else if (!strcmp (arg1, "login"))  data = 2;
        else if (!strcmp (arg1, "open"))   data = 2;
        else if (!strcmp (arg1, "select")) data = 3;
        else if (!strcmp (arg1, "delete")) data = 4;
        else if (!strcmp (arg1, "remove")) data = 4;
        else if (!strcmp (arg1, "close"))  data = 5;
        else if (!strcmp (arg1, "logoff")) data = 5;
        else                               data = 0;
    }
...
static JUMP_F(CmdUserContact)
{
    ...
    if (data)
    {
        QueueEnqueueData (conn, QUEUE_REQUEST_ROSTER, data, 0x7fffffffL, NULL, 0, NULL, NULL);
