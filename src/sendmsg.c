  ...
  if ((queued_msg = msg_queue_pop ()) != NULL)
      ...
      if (queued_msg->attempts <= MAX_RETRY_ATTEMPTS)
	{
	  if (Verbose)
	    {
	      R_undraw ();
	      M_print ("\nResending message with SEQ num %04x.\tCMD ",
		       (queued_msg->seq >> 16));
...
***************************************************/
void
Fill_Header (net_icq_pak * pak, WORD cmd)
{
...
void
Update_Other (SOK_T sok, OTHER_INFO_PTR info)
{
  net_icq_pak pak;
  BYTE *buf;
...
*************************************************/
void
Update_About (SOK_T sok, const char *about)
{
  net_icq_pak pak;
  ...
  char_conv ("cw", &pak.data[4]);
  pak.data[strlen (about) + 4] = 0;
  last_cmd[(seq_num - 1) & 0x3ff] = Chars_2_Word (pak.head.cmd);
  SOCKWRITE (sok, &(pak.head.ver),
	     sizeof (pak.head) - 2 + 5 + strlen (about));
}
...
**************************************/
void
update_list (int sok, DWORD uin, int which, BOOL add)
{
  net_icq_pak pak;
  ...
  for (i = 0; i < Num_Contacts;)
    {
      ...
      tmp = pak.data;
      tmp++;
      for (j = 0, num_used = 0; (j < MAX_CONTS_PACKET) && (i < Num_Contacts);
	   i++)
	{
  ...
  if (Verbose)
    {
      R_undraw ();
#ifdef FUNNY_MSGS
      M_print ("\nIf you go to" MESSCOL " Z'Ha'Dum " NOCOL
	       "you will die!!\n");
...
static const BYTE table[] = {
  ,
};
...
****************************************************/
void
icq_sendmsg (SOK_T sok, DWORD uin, char *text, DWORD msg_type)
{
  SIMPLE_MESSAGE msg;
  net_icq_pak pak;
  int size, len;
...
***************************************************/
void
icq_sendauthmsg (SOK_T sok, DWORD uin)
{
  SIMPLE_MESSAGE msg;
  net_icq_pak pak;
  int size;
...
****************************************************/
void
icq_rand_user_req (SOK_T sok, DWORD group)
{
  net_icq_pak pak;
  int size;
...
****************************************************/
void
icq_rand_set (SOK_T sok, DWORD group)
{
  net_icq_pak pak;
  int size;
...
****************************************************/
void
icq_change_status (SOK_T sok, DWORD status)
{
  net_icq_pak pak;
  int size;
...
void
info_req_99 (SOK_T sok, DWORD uin)
{
  net_icq_pak pak;
  int size;
...
void
info_req_old (SOK_T sok, DWORD uin)
{
  net_icq_pak pak;
  int size;
...
**********************************************************/
void
send_info_req (SOK_T sok, DWORD uin)
{
#if 0
...
**********************************************************/
void
send_ext_info_req (SOK_T sok, DWORD uin)
{
  net_icq_pak pak;
  int size;
...
****************************************************************/
void
start_search (SOK_T sok, char *email, char *nick, char *first, char *last)
{
  net_icq_pak pak;
  int size;
...
****************************************************/
void
reg_new_user (SOK_T sok, char *pass)
{
#if ICQ_VER == 0x0002
  srv_net_icq_pak pak;
#else
...
*******************************************************/
void
Change_Password (SOK_T sok, char *pass)
{
#if ICQ_VER == 0x0002
  srv_net_icq_pak pak;
#else
...
void
Update_User_Info (SOK_T sok, USER_INFO_PTR user)
{
  net_icq_pak pak;
  int size;
...
void
Update_More_User_Info (SOK_T sok, MORE_INFO_PTR user)
{
  net_icq_pak pak;
  int size;
...
void
icq_sendurl (SOK_T sok, DWORD uin, char *description, char *url)
{
  char buf[450];
...
void
Gen_Checksum (BYTE * buf, DWORD len)
{
  DWORD cc, cc2;
  DWORD r1, r2;
...
****************************************************************/
size_t
SOCKWRITE (SOK_T sok, void *ptr, size_t len)
{
...
****************************************************************/
static size_t
SOCKWRITE_LOW (SOK_T sok, void *ptr, size_t len)
{
...
size_t
SOCKREAD (SOK_T sok, void *ptr, size_t len)
{
  size_t sz;
